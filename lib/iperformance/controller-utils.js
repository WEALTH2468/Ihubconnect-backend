...
const calculateParentStatusAndProgress = (objectives) => {
  ...
  console.log({ progressPercentage, completedObjectives });
...
const initializeUserStats = (weight) => {
  return {
    ...
    weights: new Set(),
    weightCount:
      weight && Object.keys(weight).length > 0 ? { [weight._id]: 1 } : weight, // Count for the initial weight
  };
};
...
const updateObjectiveStatus = async (companyDomain, objectiveId) => {
  try {
    // get the tasks for that objective
    const objectiveWithTasks = await Objective.aggregate([
      {
        $match: { _id: objectiveId, companyDomain },
      },
      {
        $lookup: {
          from: "task2",
